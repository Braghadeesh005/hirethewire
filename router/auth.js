...
router.post('/employeeregister', async(req,res)=>{
    ...
    const { name, email, phone, work, password, cpassword, rating, scrap, amount } = req.body;
   
    if(!name || !email || !phone || !work || !password || !cpassword || !rating || !scrap || !amount)
    {
        console.log("please fill the field properly");
        return res.status(422).json({error: "please fill the field properly"});
    }
    ...
    try{ 
        ...
        const user = new User2({ name, email, phone, work, password, cpassword, rating, scrap, amount}); 
...
//to get data for employee
 router.get("/getData1",authenticate2, (req,res) => 
 {
     res.send(req.rootUser);
 });
